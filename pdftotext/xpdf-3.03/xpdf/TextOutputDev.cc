...
void TextPage::coalesce(GBool physLayout, double fixedPitch, GBool doHTML) {
		...
		for (baseIdx = pool->minBaseIdx; baseIdx <= pool->maxBaseIdx; ++baseIdx) {
			for (word0 = pool->getPool(baseIdx); word0; word0 = word0->next) {
				printf("    word: x=%.2f..%.2f y=%.2f..%.2f base=%.2f fontSize=%.2f rot=%d link=%p '",
					word0->xMin, word0->xMax, word0->yMin, word0->yMax,word0->base, word0->fontSize, rot*90, word0->link);
				for (i = 0; i < word0->len; ++i) 
				{
					printf(" %d ",word0->text[i]);
					printf(" %c",word0->text[i] & 0xff);
	...
	for (rot = 0; rot < 4; ++rot) {
		...
		while (1) {
			...
			if (poolMinBaseIdx > pool->maxBaseIdx) { break; }

			// look for the left-most word in the first four lines of the
			// pool -- this avoids starting with a superscript word
			startBaseIdx = poolMinBaseIdx;
			for (baseIdx = poolMinBaseIdx + 1 ; baseIdx < poolMinBaseIdx + 4 && baseIdx <= pool->maxBaseIdx ; ++baseIdx) {	
				if (!pool->getPool(baseIdx)) { continue; }
				if (pool->getPool(baseIdx)->primaryCmp(pool->getPool(startBaseIdx)) < 0) { startBaseIdx = baseIdx; }
			}
				...
				for (baseIdx = pool->getBaseIdx(minBase) ; baseIdx >= pool->getBaseIdx(minBase - lineSpace) ; --baseIdx) {
					...
					while (word1) {
						if (word1->base < minBase &&
							...
							((rot == 0 || rot == 2) ? 
							(word1->xMin < blk->xMax && word1->xMax > blk->xMin) : (word1->yMin < blk->yMax && word1->yMax > blk->yMin)) &&
					...
					++baseIdx) {
						...
						while (word1) {
							if (word1->base >= minBase - intraLineSpace &&
								...
								word1->xMax > blk->xMin - colSpace1) : (word1->yMin < blk->yMax + colSpace1 &&
				...
				for (baseIdx = pool->getBaseIdx(minBase - intraLineSpace);
					...
					++baseIdx) {
						...
						while (word1) {
							if (word1->base >= minBase - intraLineSpace &&
								word1->base <= maxBase + intraLineSpace &&
								((rot == 0 || rot == 2) ? (word1->xMax <= blk->xMin &&
